...
* `TokenRateLimitPolicy`: "1,500 tokens per minute per team, counted from the real `usage.total_tokens` the model returns. Not requests. Tokens."
...
. Exhaust the budget on purpose, as the team, from the Web Terminal:
+
[source,bash,role="copypaste",subs="attributes+"]
----
PK=$(oc get secret llm-key-team-claims -n kuadrant-system -o jsonpath='{.data.api_key}' | base64 -d)
...
. Back in the application Inbox, show that emails arriving during the minute of exhausted budget fall back to the keyword router (or queue), and recover on the next minute:
* "This is what a controlled failure looks like. The team hit its budget; the platform said no; nobody else was affected."
...
* The budget is counted in tokens from the model's own usage report, not in requests
